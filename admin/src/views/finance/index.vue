<template>
  <div class="page-stack">
    <div>
      ...
      <p class="page-subtitle">查看红包与消费券核销的资金概览，并支持导出。</p>
    </div>
    ...
    <div class="page-card" style="padding:16px;">
      ...
      <el-row :gutter="12">
        ...
        <el-col :span="6"><div><div>红包到账</div><strong>¥{{ summary.redPacketWithdrawn }}</strong></div></el-col>
        <el-col :span="6"><div><div>消费券核销</div><strong>¥{{ summary.couponTotal }}</strong></div></el-col>
    ...
    <div class="page-card" style="padding:16px;">
      <el-table :data="records" v-loading="loading" border>
        <el-table-column prop="type" label="类型" width="120">
          <template #default="{ row }">{{ row.type === 'redpacket' ? '红包' : '消费券' }}</template>
        </el-table-column>
...
const summary = reactive({
  redPacketIssued: '0.00',
  redPacketWithdrawn: '0.00',
  couponTotal: '0.00',
  totalExpense: '0.00'
})
...
async function exportCsv() {
  ...
  link.href = url
  link.download = `finance-${Date.now()}.csv`
  link.click()
