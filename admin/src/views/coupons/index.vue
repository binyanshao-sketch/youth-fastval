  ...
  <div class="page-stack">
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <div>
        ...
        <p class="page-subtitle">新建、筛选与停用消费券。</p>
      </div>
      <el-button type="primary" @click="show = true">新建</el-button>
    </div>
      ...
      <el-table :data="list" v-loading="loading" border>
        ...
        <el-table-column label="操作" width="100">
          <template #default="{ row }">
            <el-button link type="danger" @click="disableCoupon(row.id)">停用</el-button>
          </template>
        </el-table-column>
      </el-table>
...
const form = reactive({ name: '', amount: 20, minSpend: 50, totalCount: 100, validFrom: '2026-04-01', validTo: '2026-05-31', description: '' })
...
const rules = {
  ...
  minSpend: [{ required: true, message: '请输入门槛金额', trigger: 'change' }],
  totalCount: [{ required: true, message: '请输入总量', trigger: 'change' }],
  dateRange: [{ validator: (_, __, cb) => {
    if (!form.validFrom || !form.validTo) return cb(new Error('请选择有效期'))
    cb()
  }, trigger: 'change' }]
}
...
async function loadData(page = pagination.page) {
  ...
  loading.value = true
  try {
    const res = await api.getCoupons({ page: pagination.page, pageSize: pagination.pageSize })
    if (res.success) {
      const raw = res.data.list || []
      list.value = filters.name
        ? raw.filter(item => item.name?.includes(filters.name))
        : raw
