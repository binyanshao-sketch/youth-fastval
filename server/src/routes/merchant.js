...
router.post('/send-code', [
  ...
  try {
    ...
    if (!merchant) {
      return res.json({ success: true, message: '如该手机号已注册，验证码将发送至该号码' });
    }
    ...
    // 供应商失败时要显式返回失败，避免前端误以为验证码已发送。
    const smsResult = await SMSService.send(phone, `您的验证码是：${code}，5分钟内有效。`);
...
router.post('/login', [
  ...
  try {
    ...
    // 验证码校验：原子取出并删除，防止同一验证码被多次使用（TOCTOU）
    const luaGetDel = `
      local val = redis.call('GET', KEYS[1])
      if val then redis.call('DEL', KEYS[1]) end
      return val
    `;
    const cachedCode = await req.redis.eval(luaGetDel, 1, codeKey);
    ...
    const token = req.app.locals.jwt.sign(
      { merchantId: merchant.id, role: 'merchant', type: 'merchant' },
      '7d'
    );
