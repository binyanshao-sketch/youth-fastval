...
```bash
cd /opt
git clone https://github.com/your-repo/youth-fastval.git
cd youth-fastval
```
...
# 上传SSL证书
# festival.example.com.crt
# festival.example.com.key
# api.festival.example.com.crt
# api.festival.example.com.key
```
...
```nginx
server_name festival.example.com www.festival.example.com;
...
- 登录微信公众平台
- 配置服务器域名：`https://api.festival.example.com`
- 配置业务域名：`https://festival.example.com`
...
# 或手动执行
mysql -h 127.0.0.1 -uroot -p youth_fastval < database/schema.sql
```
...
```bash
# 备份数据库
docker-compose exec mysql mysqldump -uroot -p youth_fastval > backup_$(date +%Y%m%d).sql
